...
%
% ASSUMES add_nassar_to_group_table.m IS ON THE PATH
% (joins Nassar latents into group_T via (subjID, block, trial) key).
% ==========================================================================
...
load(fullfile(eeg_path,  'group_feature_table_combined_v9c.mat'));  % group_table_combined
...
fprintf('  sim_fig1_beliefs.pdf         — simulated belief trajectories\n');
fprintf('  sim_fig2_switch_vs_maint.pdf — accuracy aligned to reversal\n');
fprintf('  sim_fig3_alpha_reversal.pdf  — learning rate aligned to reversal\n');
fprintf('  sim_fig4_alpha_by_encounter.pdf — α and ω per-stimulus encounter\n');
fprintf('  fit_fig_surprise_aligned.pdf — grand-average surprise (from §5)\n');
fprintf('  fig1_param_distributions.pdf — H and ΔH histograms\n');
fprintf('  fig2_param_vs_behaviour.pdf  — parameter–behaviour correlations\n');
fprintf('  fig3_model_fit.pdf           — NLL and BIC\n');
fprintf('  fig4_uncertainty_vs_confidence.pdf\n');
fprintf('  fig5_diag_<subjID>.pdf       — per-subject α / ω diagnostics\n');
fprintf('  fig6_param_recovery.pdf      — parameter recovery scatter\n');
fprintf('  fig7_model_comparison.pdf    — Nassar vs RW BIC\n');
fprintf('  fig8_ppc.pdf                 — posterior predictive check\n');
fprintf('  fig_blocktype_H.pdf          — H_det vs H_prob (from §6)\n');
fprintf('\nSaved data:\n');
...
fprintf('    → loaded by F_RL_EEG_analysis.m\n');
fprintf('  %s\n', fullfile(eeg_path,'group_feature_table_combined_v9c_RL.mat'));
fprintf('    → group_T  (group_table_combined + Nassar latents)\n');
fprintf('    → New columns: PE_nassar, omega, alpha_nassar,\n');
fprintf('                   certainty, surprise, theta_nassar\n');
fprintf('    → switch_stims column preserved from input table\n');
fprintf('========================================================\n\n');
...
% Block structure matches real task (P P D P D)
STRUCTURE    = 'PPDPD';
P_TRUE_FB    = [0.8, 0.8, 1.0, 0.8, 1.0];   % 1.0 = deterministic
...
% ── Read SWITCH_STIMS from group_T (authoritative source) ────────────────
% group_table_combined already has a switch_stims column written by the
% previous pipeline run. Read the value from the first valid non-empty row.
% This is used for: simulation figures (§4), PPC (§10), summary stats.
% It is NOT re-derived during fitting — detect_switch_stims is removed.
SWITCH_STIMS = [];
if ismember('switch_stims', group_table_combined.Properties.VariableNames)
    for ri = 1:height(group_table_combined)
        sw_candidate = group_table_combined.switch_stims{ri};
        if ~isempty(sw_candidate)
            SWITCH_STIMS = sw_candidate;
            break;
        end
    end
end
if isempty(SWITCH_STIMS)
    warning('switch_stims column not found or empty in group_T. Defaulting to [2 3].');
    SWITCH_STIMS = [2 3];
end
MAINT_STIMS = setdiff(1:size(STIM_FEATURES,1), SWITCH_STIMS);
fprintf('SWITCH_STIMS read from group_T: [%s]\n', num2str(SWITCH_STIMS));
...
% =========================================================================
%% §11 ADD NASSAR LATENTS TO GROUP_T
%
% switch_stims is already present in group_table_combined from the previous
% pipeline run — no need to re-derive or re-write it here.
%
% Assumes add_nassar_to_group_table.m is on the path.
% =========================================================================
fprintf('\n=== §11: Joining Nassar latents to group_T ===\n');

group_T = group_table_combined;

% Verify switch_stims column is present
if ~ismember('switch_stims', group_T.Properties.VariableNames)
    error(['switch_stims column not found in group_table_combined.\n' ...
           'Run the previous pipeline step that saves this column first.']);
end
fprintf('  switch_stims column verified in group_T (%d non-empty rows)\n', ...
    sum(~cellfun(@isempty, group_T.switch_stims)));

% Add Nassar latent columns via (subjID, block, trial) key join
group_T = add_nassar_to_group_table(group_T, results);

% Save enriched table
out_file = fullfile(eeg_path, 'group_feature_table_combined_v9c_RL.mat');
save(out_file, 'group_T');

fprintf('\n========================================================\n');
fprintf('§11 SAVE COMPLETE\n');
fprintf('  File: %s\n', out_file);
...
fprintf('    certainty      — |θ_t − 0.5| (prospective uncertainty)\n');
fprintf('    surprise       — ω_t × |δ_t| (retrospective surprise)\n');
fprintf('    theta_nassar   — θ_t (current belief state)\n');
fprintf('  switch_stims   — preserved from input table (not re-derived)\n');
fprintf('========================================================\n');

fprintf('\n=== All done. ===\n');
fprintf('Figures        → %s\n', outpath);
fprintf('Nassar results → %s\n', fullfile(outpath,'nassar_results.mat'));
fprintf('Enriched table → %s\n', out_file);
fprintf('Sim data       → %s\n', fullfile(outpath,'sim_data.mat'));
...
lims_bt = [0, max([H_det_bt(ok_bt); H_prob_bt(ok_bt)],'omitnan')+0.02];
