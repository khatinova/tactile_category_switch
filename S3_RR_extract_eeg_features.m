...
load('subj_rows_plot = group_table(group_table.subjID == subj & ~group_table.false_fb, :);');
...
for si = valid_participants
    ...
    has_theta = exist(fullfile(RR_epoch_folder, theta_fname),'file');
    has_phase = exist(fullfile(RR_epoch_folder, phase_fname),'file');

    theta_fname_untrimmed = sprintf('%s_outcome_theta.set', subj);
    phase_fname_untrimmed = sprintf('%s_outcome_phase.set', subj);

    if has_theta
        EEGp_theta = pop_loadset(theta_fname, RR_epoch_folder);
    else
        EEGp_theta = pop_loadset(theta_fname_untrimmed, RR_epoch_folder);
    end
    if has_phase
        EEGp_phase = pop_loadset(phase_fname, RR_epoch_folder);
    else
