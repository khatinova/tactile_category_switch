...
gt_true = gt(~gt.false_fb & ...
              ~isnan(gt.FCz_neg_peak_norm_z) & ...    % FIX 5: use _z version
              ~isnan(gt.P300_amp_z), :);
...
% -------------------------------------------------------------------------
%% NEW PLOTS: FCz negative peak and P300
% -------------------------------------------------------------------------

plot_stage_outcome_lines(gt, 'FCz_neg_peak_norm_z', ...
    'FCz negative peak (z)', ...
    'RQ: FCz negative peak across stages — correct vs incorrect', ...
    figure_output_folder, 'FCz_negpeak_stage_correct_incorrect');

plot_stage_transition_feedback(gt, 'FCz_neg_peak_norm_z', ...
    'FCz negative peak (z)', ...
    'RQ: FCz negative peak across stages — D/P and true/false feedback', ...
    figure_output_folder, 'FCz_negpeak_stage_block_feedback');
...
gt_inc = gt(...%gt.correct==0 & ~gt.false_fb & ...
             ~isnan(gt.confidence) & ~isnan(gt.FCz_neg_peak_norm_z), :);
...
mdl_fn_conf = fitlme(gt_inc, ...
    ['FCz_neg_peak_norm_z ~ conf_z * block_type + stage + ' ...
     '(1 + conf_z | subj_id)'], ...
    'FitMethod','REML');
...
gt_p = gt(gt.block_type=='P' & ...
           ~isnan(gt.FCz_neg_peak_norm_z) & ~isnan(gt.confidence), :);
...
if height(gt_p) > 10
    mdl_false_fn = fitlme(gt_p, ...
        ['FCz_neg_peak_norm_z ~ conf_z * false_fb_cat + stage + ' ...
         '(1 | subj_id)'], ...
        'FitMethod','REML');
...
for bt_i = 1:2
    ...
    mask = gt_inc.block_type==bt{bt_i} & ...
           ~isnan(gt_inc.conf_z) & ~isnan(gt_inc.FCz_neg_peak_norm_z);
    if sum(mask) < 5; continue; end

    x = gt_inc.conf_z(mask);
    y = gt_inc.FCz_neg_peak_norm_z(mask);
    ok = ~isnan(x) & ~isnan(y);
