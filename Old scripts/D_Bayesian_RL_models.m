...
for si = 1:N_subj
    ...
    for b = 1:min(nB4,5)
        ...
        sw_b = ternary(isfield(r,'switch_stims_by_block') && b <= numel(r.switch_stims_by_block), ...
            r.switch_stims_by_block{b}, SWITCH_STIMS);
    ...
    exportgraphics(fig_d, fullfile(outpath, sprintf('fig5_diag_%s.pdf',sn)), 'PaperPosition', 'landscape');
    %close(fig_d);
end
...
lims_mc = [min(all_b)-20, max(all_b)+20];
...
%
% Assumes add_nassar_to_group_table.m is on the path.
% Also writes switch_stims (detected per subject from data) as a cell
% column: every row for subject sn gets the same switch_stims vector.
% This allows D_Individual_differences to access it from any row.
% =========================================================================
fprintf('\n=== §11: Joining Nassar latents and switch_stims to group_T ===\n');

group_T = group_table_combined;

% Add Nassar latent columns
group_T = add_nassar_to_group_table(group_T, results);

% Add switch_stims column (cell array: each cell holds the integer vector
% of switched stimulus IDs for that subject, e.g. [2 3])
group_T.switch_stims = cell(height(group_T), 1);

for si = 1:numel(subj_ids)
    sn = subj_ids{si};
    r  = results.(sn);

    % Aggregate switch stims across blocks (should be identical per block
    % for a dimensional-shift design; take the union as a safety net)
    if isfield(r,'switch_stims_by_block')
        sw_all = unique(horzcat(r.switch_stims_by_block{:}));
    else
        sw_all = SWITCH_STIMS;
    end

    row_mask = string(group_T.subj_id) == string(sn);
    for ri = find(row_mask)'
        group_T.switch_stims{ri} = sw_all;
    end
end

% Save enriched table
save(fullfile(eeg_path, 'group_feature_table_combined_v9c_RL.mat'), 'group_T');
fprintf('Saved enriched table: %d rows, %d cols\n', height(group_T), width(group_T));
fprintf('New columns added: PE_nassar, omega, alpha_nassar, certainty, surprise, theta_nassar, switch_stims\n');
...
for si = 1:numel(subjects)
    ...
    % Detect switched stimuli from data
    sw_info = detect_switch_stims(td, sf, size(stim_features,1));
    ...
    results.(sn).switch_stims_by_block   = sw_info.switch_stims_by_block;
    results.(sn).maint_stims_by_block    = sw_info.maint_stims_by_block;
...
lims_bt = [0, max([H_det_bt(ok_bt); H_prob_bt(ok_bt)])];
...
function sw_info = detect_switch_stims(td, stim_field, nStim)
[nBlocks, nTrials] = get_n_blocks_trials(td);
sw_info.switch_stims_by_block = cell(1, nBlocks);
sw_info.maint_stims_by_block  = cell(1, nBlocks);
if ~isfield(td,'goTrial') || ~isfield(td,'revTrial') || ~isfield(td,stim_field)
    return;
end
for b = 1:nBlocks
    rev = td.revTrial(b);
    if isnan(rev) || rev < 1 || rev >= nTrials, continue; end
    rev = round(rev);
    stim_vec = td.(stim_field)(b,:);
    go_vec   = td.goTrial(b,:);
    sw = []; mn = [];
    for s = 1:nStim
        pre_g  = go_vec(stim_vec==s & (1:nTrials)<=rev);
        post_g = go_vec(stim_vec==s & (1:nTrials)>rev);
        pre_g  = pre_g(~isnan(pre_g)); post_g = post_g(~isnan(post_g));
        if isempty(pre_g) || isempty(post_g), continue; end
        if round(mean(pre_g)) ~= round(mean(post_g)), sw(end+1) = s;
        else, mn(end+1) = s; end
    end
    sw_info.switch_stims_by_block{b} = sw;
    sw_info.maint_stims_by_block{b}  = mn;
end
end
...
subjs = categories(categorical(group_T.subjID));
