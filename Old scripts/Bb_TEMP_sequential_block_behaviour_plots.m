% ==========================================================================
% E_sequential_block_behaviour_plots_aligned.m
%
...
load(fullfile(data_path, 'behav_table_June2026.mat'), 'group_T');
...
% ── Load Nassar2010 results from the modelling output ───────────────
% Behavioural switch-stimulus labels are taken from group_T below.
% The Nassar results are loaded only for model-latent overlays / consistency
% checks and should come from D_Bayesian_RL_models_behaviour_only.m.
if ~exist('results','var')
    results = [];
    nassar_candidates = { ...
        fullfile(base_path,'Results','Simulation results','Figures','nassar_results.mat'), ...
        fullfile(base_path,'Results','Simulation results','nassar_results.mat'), ...
        fullfile(outpath,'nassar_results.mat')};

    for ci = 1:numel(nassar_candidates)
        if exist(nassar_candidates{ci},'file')
            try
                tmp = load(nassar_candidates{ci});
                if isfield(tmp,'results')
                    results = tmp.results;
                    fprintf('Loaded Nassar2010 results from %s\n', nassar_candidates{ci});
                    break;
                end
            catch ME_load_results
                warning('Could not load Nassar2010 results from %s: %s', ...
                    nassar_candidates{ci}, ME_load_results.message);
            end
        end
    end

    if isempty(results)
        fprintf(['Note: Nassar2010 results not found. Expected file: %s\n' ...
                 'Behavioural plots will still run because switch_stims are read from group_T.\n'], ...
            fullfile(base_path,'Results','Simulation results','Figures','nassar_results.mat'));
    end
end
...
if ~ismember('switch_stims', group_T.Properties.VariableNames)
    warning(['group_T.switch_stims was not found. The script will fall back to [%s]. ' ...
             'For the intended analysis, regenerate behav_table_June2026.mat with switch_stims.'], ...
        num2str(SWITCH_STIMS_DEFAULT));
else
    fprintf('Using group_T.switch_stims as the authoritative switched-stimulus source.\n');
end
...
for si = 1:N_subj
    ...
    for bi = 1:numel(blocks_s)
        ...
        % Authoritative switched-stimulus labels come from group_T.switch_stims.
        % This supports numeric, string, categorical, or cell-valued columns.
        sw_stims = get_switch_stims_from_group_T_lc(group_T, char(sn), b, SWITCH_STIMS_DEFAULT);
...
meta_transition = {};
meta_cohort    = {};
meta_stim_config = {}; % stimulus configuration per block (a/b/c/d/e when available)
...
for si = 1:numel(subj_ids_td)
    ...
    % Switch stims are read from group_T at the subject × block level.
    % Do not use results.switch_stims_by_block here: the behavioural table is
    % the authoritative source and is shared with RR/Ox combined behaviour.
    ...
    for b = 1:nB
        ...
        % Determine switched/maintained stimuli from group_T.switch_stims.
        sw_b = get_switch_stims_from_group_T_lc(group_T, sn_char, b, SWITCH_STIMS_DEFAULT);
...
% Force all row-level metadata to column vectors. This prevents MATLAB
% implicit expansion from turning row masks into N x N logical matrices.
...
assert(size(conf_rows,1) == numel(meta_block), 'conf_rows and metadata row counts differ');
assert(size(acc_sw_rows,1) == numel(meta_block), 'acc_sw_rows and metadata row counts differ');
...
function sw = get_switch_stims_from_group_T_lc(group_T, sn_char, b, default_sw)
%GET_SWITCH_STIMS_FROM_GROUP_T_LC  Subject×block switch-stim lookup.
% The behavioural table is the authoritative source for switched stimuli.
% Handles cell-valued rows like {[2 3]}, numeric matrices, strings such as
% "2 3" / "[2,3]", and categorical/string columns.
sw = default_sw(:)';
try
    if ~istable(group_T) || ~ismember('switch_stims', group_T.Properties.VariableNames)
        return;
    end
    if ~ismember('subjID', group_T.Properties.VariableNames) || ...
    ...
    vals = group_T.switch_stims(rows,:);
    parsed = [];

    if iscell(vals)
        for ii = 1:numel(vals)
            parsed = parse_switch_stims_value_lc(vals{ii});
            if ~isempty(parsed), break; end
        end
    ...
    elseif isstring(vals) || iscategorical(vals)
        ...
        if ~isempty(vals)
            parsed = parse_switch_stims_value_lc(vals(1));
        end
    elseif ischar(vals)
        parsed = parse_switch_stims_value_lc(vals);
    end

    parsed = double(parsed(:)');
    parsed = parsed(isfinite(parsed) & parsed >= 1 & parsed <= 4);
    parsed = unique(round(parsed), 'stable');
    if ~isempty(parsed)
        sw = parsed;
    end
catch ME_sw
    warning('Could not read switch_stims for %s block %d from group_T: %s', ...
        char(string(sn_char)), b, ME_sw.message);
    sw = default_sw(:)';
end
end

function sw = parse_switch_stims_value_lc(val)
%PARSE_SWITCH_STIMS_VALUE_LC  Normalise one switch_stims table value.
sw = [];
...
try
    ...
    if isstring(val)
        if any(ismissing(val)), return; end
        val = char(val(1));
    end
    if ischar(val)
        str = strtrim(val);
        if isempty(str) || any(strcmpi(str, {'nan','na','none','missing','unknown'}))
            return;
        end
...
function make_axes_square_lc(fig_h)
%MAKE_AXES_SQUARE_LC  Presentation cleanup: outward ticks and square axes.
% Applied immediately before saving each figure so helper plotting calls cannot
% overwrite the final appearance.
try
    ax_all = findall(fig_h, 'Type', 'axes');
    for ai = 1:numel(ax_all)
        ax = ax_all(ai);
        try
            ...
            % Some annotation or special axes may not accept these properties.
        end
    end
catch ME_sq
    warning('Could not square axes for figure: %s', ME_sq.message);
end
end
