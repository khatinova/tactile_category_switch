...
mdl_fn_conf = fitlme(gt_inc, ...
    ['prefrontal_neg_peak_norm_z ~ conf_z * block_type + stage + ' ...
     '(1 + conf_z | subj_id)'], ...
    'FitMethod','REML');
fprintf('  FRN ~ confidence × block_type (all trials):\n');
...
sgtitle('RQ2: Confidence × Feedback negativity (all trials)');
bt = {'D','P'};
outcome = [1,0];
for bt_i = 1:2
...
exportgraphics(fig,fullfile(outdir,'RQ2_Confidence_alltrials_FN.pdf'),'ContentType','vector');
% close(fig);
