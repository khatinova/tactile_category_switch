% =============================================================================
% MORLET WAVELET TIME-FREQUENCY ANALYSIS — KH COHORT
%
...
%
% OUTPUTS:
%   1. TF maps (ERSP and ITPC) per subject, per stage, per block type,
%      per outcome condition — saved as PDFs.
%   2. Grand-average TF maps across participants — saved as PDFs.
%   3. Single-trial feature columns appended to group_table and saved as
%      group_stage_table_wavelet.mat:
%         .theta_ersp    — mean dB power in theta band × FRN window
%         .alpha_ersp    — mean dB power in alpha band × P300 window
%         .beta_ersp     — mean dB power in beta band × post-feedback window
%         .theta_itpc    — mean ITPC in theta band × FRN window
%         .theta_ersp_z  — within-subject z-scored theta_ersp
%         .theta_itpc_z  — within-subject z-scored theta_itpc
%   4. Cluster-based permutation tests (FieldTrip) on the 2D TF plane for
%      correct vs incorrect, and D vs P block types.
%      Results saved as tf_perm_stats.mat.
%
% WHAT TO EXPECT (see wavelet_TF_analysis_KH.m header for full discussion):
%   - Theta (4-8 Hz) increase at FCz after incorrect outcomes, ~250-400 ms.
%     This is the oscillatory signature of your FRN.
...
%   Mas-Herrero & Marco-Pallarés (2014) J Cogn Neurosci 26:447-458.
%
% REQUIRES: FieldTrip on path (for permutation tests).
%           group_stage_table_v7.mat from outcome_ERP_analysis_v7.m.
% =============================================================================
...
figure_output_folder = '\\humerus\pharm_banerjee\data\Projects\EEG_projects\Salient_Modality_Switch\Salient mod switch KH\Results\EEG analysis\Figures\TF_analysis';
...
% -------------------------------------------------------------------------
%% LOAD PRE-BUILT GROUP TABLE (from outcome_ERP_analysis_v7.m)
% -------------------------------------------------------------------------
load(fullfile(epoch_file_folder,'group_stage_table_v7.mat'), 'group_table');
fprintf('Loaded group_table: %d rows, %d subjects\n', ...
    height(group_table), numel(unique(group_table.subj)));
...
%
% Each leaf stores: .ersp (n_subj × n_freq × n_time) and .subj (n_subj × 1)
...
for participant = valid_participants
    ...
    subj_rows = group_table.subj == categorical(participant);
    fprintf('\n=== %s ===\n', subj);
    ...
    acc_idx = safe_chan(EEGp, CHANNELS.Acc);
    ...
    %
    % tf_power : n_freq × n_time × n_epoch  (raw power, before baseline)
    % tf_phase : n_freq × n_time × n_epoch  (phase angles, radians)
    % ------------------------------------------------------------------
    n_times  = size(EEGp.data, 2);
    n_epochs = EEGp.trials;

    if ~isempty(fcz_idx)
        fprintf('  Computing wavelet TF at FCz (%d freqs × %d times × %d epochs)...\n', ...
        ...
        for bt_i = 1:2
            ...
            for oc_i = 1:2
                ...
                if ~isempty(tf_power_fcz)
                    ...
                    %
                    % For each trial, compute mean ERSP in band × window.
                    ...
                    % bl_mean: N_FREQS × 1 (grand baseline across all epochs for this subject)
    ...
    %
    % One figure per subject: 4 stages × 2 outcomes (correct/incorrect),
...
% =========================================================================
%% WITHIN-SUBJECT Z-SCORE OF WAVELET FEATURES
% =========================================================================
...
save(fullfile(epoch_file_folder,'group_stage_table_wavelet.mat'), 'group_table');
fprintf('Saved group_stage_table_wavelet.mat\n');

save(fullfile(epoch_file_folder,'grand_tf.mat'), 'grand_tf', 't_global', 'frex');
...
for s_i = 1:4
    ...
    for bt_i = 1:2
        ...
        for oc_i = 1:2
            ...
            if ~isempty(g.ersp)
                ...
                % n_subj × n_freq × n_time → freq × time after mean
    ...
    saveas(fig, fullfile(figure_output_folder, ...
        sprintf('v7_TF_grand_%s.pdf', stage_names{s_i})));
    % close(fig);
end
...
saveas(fig_diff, fullfile(figure_output_folder,'v7_TF_diff_IncVsCor.pdf'));
close(fig_diff);

% ==========================================================================
% REPLACEMENT: TF CLUSTER PERMUTATION TEST SECTION
%
% Drop this into wavelet_TF_analysis_KH.m to replace the block that starts
%   "=== TF CLUSTER PERMUTATION TESTS ===" and ends before
%   "=== THETA ERSP STAGE SUMMARY ==="
%
% WHY THE ORIGINAL FAILED
% -----------------------
% ft_freqstatistics internally calls ft_prepare_neighbours, which expects
% an EEG channel dimension with spatial coordinates (chanlocs). When the
% data is freq × time with no channel, FieldTrip cannot construct the
% adjacency graph it needs for cluster correction and throws:
%   "cannot figure out how to construct neighbours, please specify
%    cfg.neighbours or cfg.method and call this function directly"
%
% Setting cfg.minnbchan = 0 does not suppress this call.
%
% THE FIX
% -------
% Implement the 2-D cluster permutation test directly, exactly as FieldTrip
% does internally (Maris & Oostenveld 2007, J Neurosci Methods 164:177-190)
% but operating natively on the freq × time plane:
%
%   1. Compute a paired t-statistic at every (freq, time) point.
%   2. Threshold at |t| > t_crit (alpha = 0.05 two-tailed, df = n-1).
%   3. Find connected components in the thresholded map using 8-connectivity
%      (adjacent in freq, adjacent in time, or diagonally adjacent).
%      This defines clusters in the 2-D TF plane.
%   4. The observed cluster statistic is the sum of t-values within each
%      cluster (Maris & Oostenveld 2007, Eq. 2).
%   5. Under the null hypothesis of no condition difference, the sign of
%      each subject's difference map can be randomly flipped (sign-flip
%      permutation, appropriate for a paired design).
%      Repeat N_PERM times, recompute clusters, store the maximum cluster
%      statistic each time → null distribution.
%   6. p-value = proportion of null statistics ≥ observed cluster statistic.
%
% The output structure tf_perm_stats.(contrast_name) mirrors what
% plot_tf_cluster_result() expects:
%   .stat                   — freq × time t-statistic map
%   .mask                   — freq × time logical mask (sig at p < alpha_clust)
%   .posclusters(k).prob    — p-value of k-th positive cluster
%   .negclusters(k).prob    — p-value of k-th negative cluster
%   .posclusterslabelmat    — freq × time integer map, positive clusters
%   .negclusterslabelmat    — freq × time integer map, negative clusters
% ==========================================================================
...
% ── Contrast A: Correct vs Incorrect at each stage × block_type ─────────────
for s_i = 1:4
    for bt_i = 1:2
        bt = BTYPE_LABELS{bt_i};
        ...
        % diff_maps: n_subj × n_freq × n_time  (incorrect − correct)
        diff_maps = gc.ersp(ia,:,plot_t_mask) - ...   % NOTE: inc minus cor
                    gi.ersp(ib,:,plot_t_mask);
        % Flip sign so positive clusters = incorrect > correct
        diff_maps = -diff_maps;
...
% ── Save ─────────────────────────────────────────────────────────────────────
save(fullfile(epoch_file_folder,'tf_perm_stats.mat'), 'tf_perm_stats');
fprintf('\nTF permutation stats saved to tf_perm_stats.mat\n');


% ==========================================================================
% LOCAL HELPER FUNCTIONS
% (add these to the bottom of wavelet_TF_analysis_KH.m alongside the
...
%
%   stat = run_tf_cluster_permtest(diff_maps, n_perm, alpha_clust)
%
%   diff_maps : n_subj × n_freq × n_time  (condition A minus condition B,
...
%   This is equivalent to randomly swapping A and B labels for that subject.
%
%   CLUSTER DEFINITION:
%   8-connectivity is used in the freq × time plane — i.e. two points are
%   in the same cluster if they are adjacent horizontally, vertically, or
%   diagonally. This is consistent with FieldTrip's default 2-D clustering.
...
[neg_label, neg_stats] = find_clusters(-t_obs, t_crit);   % t < -t_crit (flip sign)
...
for p = 1:n_perm
    % Randomly flip sign for each subject (±1 vector, n_subj × 1 × 1)
    signs = sign(randn(n_subj, 1, 1));     % each ±1 with p=0.5
    perm_maps = diff_maps .* signs;        % broadcast across freq × time
...
for k = 1:n_pos_obs
    if posclusters(k).prob < alpha_clust
        sig_mask = sig_mask | (pos_label == k);
    end
end
for k = 1:n_neg_obs
    if negclusters(k).prob < alpha_clust
        sig_mask = sig_mask | (neg_label == k);
    end
end
...
%PAIRED_TMAP  Paired t-statistic at each freq × time point.
%
%   diff_maps : n_subj × n_freq × n_time
%
%   t = mean(diff) / (std(diff) / sqrt(n))
%   Each subject contributes one difference value at each point.

n_subj = size(diff_maps, 1);
mn     = mean(diff_maps, 1, 'omitnan');   % 1 × n_freq × n_time
sd     = std(diff_maps,  0, 1, 'omitnan');

se     = sd / sqrt(n_subj);
t_map  = squeeze(mn ./ max(se, 1e-10));   % n_freq × n_time
...
%
%   label_map    : n_freq × n_time integer matrix (0 = not in any cluster)
%   cluster_stats: 1 × n_clusters vector of summed t-values (cluster mass)
%
%   8-connectivity: two suprathreshold points belong to the same cluster
%   if they are adjacent in any of the 8 directions (including diagonals).
%   This is the standard used by both FieldTrip and SPM for cluster tests.
...
% bwconncomp with 8-connectivity (conn=8 for 2D)
...
saveas(fig_summary, fullfile(figure_output_folder,'v7_TF_theta_stage_summary.pdf'));
close(fig_summary);
...
%
% morlet_wavelet_tf: computes the full time-frequency decomposition.
%
% INPUTS:
%   sig       — n_times × n_epochs  (double, mean-centred recommended)
%   Fs        — sampling rate (Hz)
%   frex      — 1 × N_FREQS  frequency vector (Hz)
%   n_cycles  — 1 × N_FREQS  number of cycles per frequency
%
% OUTPUTS:
%   tf_power  — N_FREQS × n_times × n_epochs  (instantaneous power, µV²)
%   tf_phase  — N_FREQS × n_times × n_epochs  (instantaneous phase, rad)
%
% HOW IT WORKS:
%   For each frequency f_i:
%     1. Build a complex Morlet wavelet:
%           w(t) = A × exp(2πi·f·t) × exp(−t²/2σ²)
%        where σ = n_cycles(i) / (2π·f_i)  and  A = normalisation.
%     2. Convolve signal with wavelet using FFT-based convolution
%        (multiply in frequency domain = convolve in time domain).
%     3. Trim convolution edges (half the wavelet length on each side)
%        to remove edge artefacts.
%     4. Extract power (abs²) and phase (angle).
%
%   The number of cycles controls the Gaussian width:
%     - More cycles → narrower Gaussian → better frequency resolution
%                  → worse temporal resolution (more spread in time)
%     - Fewer cycles → wider Gaussian → better temporal resolution
%                  → worse frequency resolution (blurred in frequency)
%   This is a fundamental trade-off (Heisenberg uncertainty principle).
% =========================================================================
...
% FFT of signal (all epochs simultaneously)
% Zero-pad to next power of 2 for speed. We will trim after convolution.
nConv_base = n_times * 2 - 1;
nConv      = 2^nextpow2(nConv_base);   % for FFT speed

sig_fft = fft(sig, nConv, 1);   % nConv × n_epochs

% Wavelet time axis: centred on zero, long enough to contain the Gaussian
% at the lowest (widest) frequency.
...
%
% INPUTS:
%   cond_power — N_FREQS × n_times × n_trials
%   bl_mask    — logical, n_times × 1
%
% OUTPUT:
%   ersp       — N_FREQS × n_times  (mean across trials, in dB)
%
% dB formula: 10 × log10(power / mean_baseline_power)
% A value of +3 dB means power has doubled relative to baseline.
% A value of -3 dB means power has halved relative to baseline.
% =========================================================================
function ersp = compute_ersp(cond_power, bl_mask)

% Mean power across trials: N_FREQS × n_times
mean_power = mean(cond_power, 3, 'omitnan');

% Baseline mean for each frequency: N_FREQS × 1
bl_power = mean(mean_power(:, bl_mask), 2, 'omitnan');
...
% One figure: 4 stages × (ERSP correct, ERSP incorrect, ITPC)
...
saveas(fig, fullfile(figure_output_folder, sprintf('%s_TF_v7.pdf', subj)));
close(fig);
end
...
% =========================================================================
function plot_tf_cluster_result(stat, frex, t_plot, name, outfolder, PLOT_WIN)
...
if isfield(stat,'posclusters')
    for c = 1:numel(stat.posclusters)
        if stat.posclusters(c).prob < 0.025
            sig_mask = sig_mask + double(stat.posclusterslabelmat == c);
        end
    end
end
if isfield(stat,'negclusters')
    for c = 1:numel(stat.negclusters)
        if stat.negclusters(c).prob < 0.025
            sig_mask = sig_mask - double(stat.negclusterslabelmat == c);
        end
    end
end
...
saveas(fig, fullfile(outfolder, sprintf('v7_TF_cluster_%s.pdf', name)));
close(fig);
end


% =========================================================================
%% BUILD FIELDTRIP TIMELOCK STRUCTURE FOR TF DATA
% =========================================================================
function tl = make_ft_timelock_tf(ersp_freq_time, frex, t_vec)
% Wraps a freq × time matrix into the FieldTrip structure expected by
% ft_freqstatistics. The 'freq' dimension plays the role of the
% spatial/channel dimension, and 'time' is the temporal dimension.

tl            = [];
tl.label      = cellfun(@(f) sprintf('%.1fHz',f), num2cell(frex), 'UniformOutput',false);
tl.freq       = frex;
tl.time       = t_vec / 1000;  % FieldTrip expects seconds
tl.powspctrm  = ersp_freq_time;  % freq × time
tl.dimord     = 'freq_time';
end


% =========================================================================
%% CUSTOM TF COLORMAP (blue-white-red, perceptually uniform)
%
% Blue = power decrease, white = no change, red = power increase.
% This is the standard colourmap for ERSP plots in the literature.
% =========================================================================
