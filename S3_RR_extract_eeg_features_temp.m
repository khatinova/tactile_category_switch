% =============================================================================
% S3_extract_eeg_features.m  (was: EEG_stats_table_formulation_outcome_v3.m)
%
% PIPELINE STEP 3 of 7 — build the EEG feature table from epoched outcome data.
%
...
%     prefrontal_mean_amp/_norm          FCz/Cz fixed-window mean amplitude
...
%
% INPUTS : group_stage_table_<cohort>.mat (per-trial spine w/ epoch + stage),
%          + outcome/theta/phase .set epochs (from S2)
% OUTPUTS: group_stage_table_features_<cohort>.mat  (per-trial features)
%          frn_rewp_by_stage_<cohort>.mat           (per-stage FRN/RewP)
% =============================================================================
...
save_folder = '\\humerus\pharm_banerjee\data\Projects\EEG_projects\Salient_Modality_Switch\Salient mod switch KH\Results\EEG analysis\Outcome_feature_tables_v4_merged';

load("\\humerus\pharm_banerjee\data\Projects\EEG_projects\Salient_Modality_Switch\Salient mod switch RR\Results\EEG analysis\Outcome_feature_tables_v4_merged\group_feature_table_combined_RR.mat");

group_table = all_trials_table;
% -------------------------------------------------------------------------
%% SETTINGS
% -------------------------------------------------------------------------
valid_participants = [1:15];
...
% Feature columns
group_table.prefrontal_neg_peak_amp   = nan(n_rows,1);   % single-trial FN (min in FRN win)
group_table.prefrontal_neg_peak_norm  = nan(n_rows,1);
group_table.prefrontal_mean_amp     = nan(n_rows,1);   % FCz/Cz fixed-window mean (per trial)
...
group_table.prefrontal_signal        = cell(n_rows,1);   % per-trial FCz/Cz waveform (for difference waves)
group_table.P300_signal         = cell(n_rows,1);
group_table.Theta_signal        = cell(n_rows,1);
group_table.prefrontal_neg_peak_lat  = nan(n_rows,1);
...
for si = valid_participants
    ...
    subj_rows = group_table.subjID == subj;

    fprintf('  %s (%d trials)...\n', subj, sum(subj_rows));

    fname = sprintf('%s_outcome.set', subj);
    fpath = fullfile(RR_epoch_folder, fname);


    % Load outcome .set ONCE
    EEGp = pop_loadset(fname, RR_epoch_folder);
    if isempty(t_global)
        t_global = EEGp.times;
    end
    ...
    EEGp_theta = pop_loadset(theta_fname_untrimmed, RR_epoch_folder);
    EEGp_phase = pop_loadset(theta_fname_untrimmed, RR_epoch_folder);


    % Channel indices
    fcz_idx = find(strcmpi({EEGp.chanlocs.labels}, 'E11'), 1);
    cz_idx  = find(strcmpi({EEGp.chanlocs.labels}, 'E129'), 1);
    prefrontal_idx = find(ismember(lower({EEGp.chanlocs.labels}), lower(string(prefrontal_channels))));
    ...
    for ri = 1:numel(row_indices)
        ...
        if ~isempty(prefrontal_idx) && ~isnan(bline_rms)
            sig_fcz = mean(double(EEGp.data(prefrontal_idx, :, ep)), 1, 'omitnan');
            sig_fcz = sig_fcz - mean(sig_fcz(bl_mask), 'omitnan');
            group_table.prefrontal_neg_peak_amp(r)  = min(sig_fcz(frn_mask));
            group_table.prefrontal_neg_peak_norm(r) = group_table.prefrontal_neg_peak_amp(r) / bline_rms;
        end

        if ~isempty(prefrontal_idx) 
            sig_frn = mean(double(EEGp.data(prefrontal_idx, :, ep)), 1, 'omitnan');
            ...
            group_table.prefrontal_signal{r} = sig_frn;
            ...
            group_table.prefrontal_mean_amp(r)  = mean(sig_frn(frn_mask), 'omitnan');
    ...
    if ~isempty(prefrontal_idx) && ~isnan(bline_rms)
        subj_rows_plot = group_table(group_table.subjID == subj & ~group_table.false_fb, :);
...
%   FRN_amp  = mean over FRN window  of (incorrect - correct)  [negative]
%   RewP_amp = mean over RewP window of (correct - incorrect)  [positive]
% Built from the per-trial FCz/Cz waveforms (prefrontal_signal) + global time axis.
% ------------------------------------------------------------------
frn_rewp_opts = struct('wave_col','prefrontal_signal', ...
                       'FRN_win',[250 300], 'RewP_win',[250 350]);
frn_rewp_stage_table = kh_compute_frn_rewp_by_stage(group_table, t_global, frn_rewp_opts);

save(fullfile(save_folder,'group_stage_table_features_RR.mat'), 'group_table', 't_global');
save(fullfile(save_folder,'frn_rewp_by_stage_RR.mat'), 'frn_rewp_stage_table');
