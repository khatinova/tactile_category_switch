% =============================================================================
% S3_RR_extract_eeg_features.m  (RR-locked twin of S3_extract_eeg_features.m)
%
% PIPELINE STEP 3 of 7 (RR COHORT) — build the EEG feature table from the
% epoched RR outcome data produced by S2_RR_preprocess_epoch_eeg.m.
%
% This is a FAITHFUL PARALLEL of the KH S3 script. Every extraction step and
% local function is identical to S3_extract_eeg_features.m. The ONLY differences
% are the RR-specific settings:
%   * Paths          : RR results tree (RR Epoched_data + RR figures).
%   * Subject naming : 'Nc%02d'   (KH used 'Ox%02d').
%   * Channels       : E11 (FCz), E7 (Cz), parietal E62/E67/E72,
%                      ACC E11/E6/E16, somatosensory E36/E104/E41/E103.
%   * Participants   : RR_VALID  (see SETTINGS).
%   * Input spine    : RR per-trial spine (epoch + stage) from S2_RR.
%   * Outputs        : *_RR.mat names so they never overwrite KH outputs.
%
% Loads each subject's outcome .set ONCE and extracts:
%   PER-TRIAL (single-trial; stored in group_table):
%     FCz_neg_peak_amp/_norm/_lat   single-trial frontal negativity (min in FRN win)
%     FCzCz_mean_amp/_norm          frontocentral fixed-window mean amplitude
%     P300_amp/_norm, Theta_amp, PLV_fp/_fs(_pairwise)
%   NOTE: feature COLUMN names are kept identical to KH (FCz_*/FCzCz_*) so the
%   KH and RR tables stack cleanly in S4. For RR these are computed from the
%   EGI frontocentral electrodes E11 (FCz) and E7 (Cz) — only the channel
%   lookup differs, not the column names.
%   PER-STAGE difference waves (in frn_rewp_stage_table):
%     FRN_amp  = mean over FRN window  of (incorrect - correct)   [negative]
%     RewP_amp = mean over RewP window of (correct - incorrect)   [positive]
%
% FRN/RewP are difference waves and CANNOT be single-trial. They are computed
% per subj_id x block_type x stage via pipeline/utils/kh_compute_frn_rewp_by_stage.m.
%
% INPUTS : RR per-trial spine (epoch + stage) from S2_RR,
%          + RR outcome/theta/phase .set epochs (from S2_RR)
% OUTPUTS: group_stage_table_features_RR.mat  (per-trial features)
%          frn_rewp_by_stage_RR.mat           (per-stage FRN/RewP)
% =============================================================================

% clear; close all; clc;
close all

% Put pipeline utils on the path (subject-id, FRN/RewP helper, figure style)
...
epoch_file_folder = fullfile(base_path, 'Salient mod switch RR', 'Results', 'EEG analysis', 'Epoched_data_noisefiltering');
figure_output_folder = fullfile(base_path, 'Salient mod switch RR', 'Results', 'EEG analysis', 'Figures', 'RQ_analysis');
feature_folder    = fullfile(base_path, 'Salient mod switch RR', 'Results', 'EEG analysis', 'Outcome_feature_tables_v4_merged');
if ~exist(figure_output_folder,'dir'), mkdir(figure_output_folder); end

% RR per-trial spine (epoch + stage) built by S2_RR. The variable inside is
% 'all_trials_table'; we load it as group_table to mirror the KH S3 input.
spine_file = fullfile(feature_folder, 'group_table_all_trials_RR.mat');
S_spine = load(spine_file);
if isfield(S_spine, 'all_trials_table')
    group_table = S_spine.all_trials_table;
elseif isfield(S_spine, 'group_table')
    group_table = S_spine.group_table;
else
    error('S3_RR: %s lacks all_trials_table/group_table.', spine_file);
end

% S3 compares group_table.subj == categorical(si); ensure subj is categorical.
if ~iscategorical(group_table.subj)
    group_table.subj = categorical(group_table.subj);
end

% -------------------------------------------------------------------------
%% SETTINGS
% -------------------------------------------------------------------------
valid_participants = [1:10];   % <-- EDIT: RR participant numbers (folders "Nc01"...)
...
% Frontocentral ROI for outcome evaluation (RR EGI labels)
frontocentral_channels = {'E11','E7'};

% Other channel groups (RR EGI labels)
...
% Feature columns
group_table.FCz_neg_peak_amp   = nan(n_rows,1);   % single-trial FN (min in FRN win)
group_table.FCz_neg_peak_norm  = nan(n_rows,1);
group_table.FCzCz_mean_amp     = nan(n_rows,1);   % FCz/Cz fixed-window mean (per trial)
group_table.FCzCz_mean_norm    = nan(n_rows,1);
...
group_table.FCzCz_signal        = cell(n_rows,1);   % per-trial FCz/Cz waveform (for difference waves)
...
for si = valid_participants
    ...
    % Channel indices (RR EGI frontocentral electrodes: E11=FCz, E7=Cz)
    fcz_idx = find(strcmpi({EEGp.chanlocs.labels}, 'E11'), 1);
    cz_idx  = find(strcmpi({EEGp.chanlocs.labels}, 'E7'), 1);
    ...
    if ~isempty(fcz_idx)
        ...
        warning('%s: E11 (frontocentral) not found, skipping baseline-dependent measures.', subj);
        bline_rms = NaN;
    end
    ...
    for ri = 1:numel(row_indices)
        ...
        if ~isempty(fcz_idx) && ~isempty(cz_idx) && ~isnan(bline_rms)
            ...
            % FCz/Cz fixed-window MEAN amplitude (single-trial, baseline-corrected).
            % NOTE: this is a single-trial frontocentral mean, NOT the FRN/RewP.
            % The FRN/RewP difference waves are built per stage below.
            group_table.FCzCz_mean_amp(r)  = mean(sig_frn(frn_mask), 'omitnan');
            group_table.FCzCz_mean_norm(r) = group_table.FCzCz_mean_amp(r) / bline_rms;
        end
...
% ------------------------------------------------------------------
group_table = kh_subject_id('standardise', group_table);

% ------------------------------------------------------------------
% PER-STAGE FRN / RewP difference waves (canonical FRN/RewP).
%   FRN_amp  = mean over FRN window  of (incorrect - correct)  [negative]
%   RewP_amp = mean over RewP window of (correct - incorrect)  [positive]
% Built from the per-trial FCz/Cz waveforms (FCzCz_signal) + global time axis.
% ------------------------------------------------------------------
frn_rewp_opts = struct('wave_col','FCzCz_signal', ...
                       'FRN_win',[250 300], 'RewP_win',[250 350]);
frn_rewp_stage_table = kh_compute_frn_rewp_by_stage(group_table, t_global, frn_rewp_opts);

save(fullfile(epoch_file_folder,'group_stage_table_features_RR.mat'), 'group_table', 't_global');
save(fullfile(epoch_file_folder,'frn_rewp_by_stage_RR.mat'), 'frn_rewp_stage_table');
fprintf('Saved group_stage_table_features_RR.mat + frn_rewp_by_stage_RR.mat\n');
...
saveas(fig_norm, fullfile(figure_output_folder,'Normalisation_comparison_RR.pdf'));
...
if exist('frn_rewp_stage_table','var') && ~isempty(frn_rewp_stage_table)
    ...
    if exist('save_fig','file')
        save_fig(fig_fr, fullfile(figure_output_folder,'FRN_RewP_difference_waves_RR'));
    else
        exportgraphics(fig_fr, fullfile(figure_output_folder,'FRN_RewP_difference_waves_RR.pdf'),'ContentType','vector');
    end
