% =============================================================================
% S10_wavelet_TF_analysis_KH_RR.m  —  PIPELINE STEP 10 (optional, KH + RR cohorts)
%
...
%           in each cohorts Epoched_data_noisefiltering/. The cluster permutation test is
...
% Output stays in the KH merged-results folder because S4 stores the combined
% KH+RR analysis table there.
saved_tables_folder  = fullfile(base_path, 'Salient mod switch KH', 'Results', 'EEG analysis', 'Outcome_feature_tables_v4_merged');
figure_output_folder = fullfile(base_path, 'Salient mod switch KH', 'Results', 'EEG analysis', 'Figures', 'TF_analysis_KH_RR');
if ~exist(figure_output_folder,'dir'), mkdir(figure_output_folder); end

% Cohort-specific epoch roots. KH and RR epochs live under different projects.
epoch_folders = struct();
epoch_folders.KH = fullfile(base_path, 'Salient mod switch KH', 'Results', 'EEG analysis', 'Epoched_data_noisefiltering');
epoch_folders.RR = fullfile(base_path, 'Salient mod switch RR', 'Results', 'EEG analysis', 'Epoched_data_noisefiltering');

% Cohorts to include in this wavelet analysis.
RUN_COHORTS = {'KH','RR'};
...
% cohort column: use the S4 label if available, otherwise infer from subj_id.
if ismember('cohort', group_table.Properties.VariableNames)
    group_table.cohort = categorical(string(group_table.cohort));
else
    sid_tmp = string(group_table.subj_id);
    coh_tmp = strings(height(group_table),1);
    coh_tmp(startsWith(sid_tmp,'Ox','IgnoreCase',true)) = "KH";
    coh_tmp(startsWith(sid_tmp,'Nc','IgnoreCase',true)) = "RR";
    group_table.cohort = categorical(coh_tmp);
end
...
%   Cohort-specific channel selection.
%   These are deliberately named FCz/Pz/Acc in the analysis containers even
%   when the RR montage uses EGI labels. For RR, FCz ~= E11 and the parietal
%   and ACC ROIs follow the S2 cohort map.
CHANNELS = struct();
CHANNELS.KH = struct( ...
    'FCz', {{'FCz'}}, ...
    'Pz',  {{'Pz','P1','P2'}}, ...
...
KH_VALID_PARTICIPANTS = [3:12, 14:23, 27:28];
RR_VALID_PARTICIPANTS = 1:15;
subject_info = build_subject_info(group_table, RUN_COHORTS, KH_VALID_PARTICIPANTS, RR_VALID_PARTICIPANTS);
...
%% SUBJECT LOOP (KH + RR)
% =========================================================================
for subject_i = 1:numel(subject_info)

    subj      = subject_info(subject_i).subj_id;          % canonical subj_id label, e.g. Ox03/Nc03
    cohort    = subject_info(subject_i).cohort;           % KH/RR
    subj_key  = string(subj);                             % unique key for grand averages
    subj_rows = group_table.subj_id == subj;
    fprintf('\n=== %s [%s] ===\n', subj, cohort);

    epoch_file_folder = epoch_folders.(cohort);

    % Load broadband outcome epochs. Prefer trimmed files, but allow the
    % untrimmed file if trimming was not saved for a subject.
    EEGp = load_first_existing_set(epoch_file_folder, { ...
        sprintf('%s_outcome_trimmed.set', subj), ...
        sprintf('%s_outcome.set', subj)});
    if isempty(EEGp)
        warning('%s: outcome .set file not found in %s; skipping.', subj, epoch_file_folder);
        continue
    end
    fprintf('  Loaded %d epochs, %d channels, Fs=%g Hz\n', ...
    ...
    % Channel indices. Uses cohort-specific labels but keeps the analysis
    % containers named FCz/Pz/Acc for cross-cohort comparability.
    chmap = CHANNELS.(cohort);
    fcz_idx = safe_chan(EEGp, chmap.FCz);
    pz_idx  = safe_chan(EEGp, chmap.Pz);
    acc_idx = safe_chan(EEGp, chmap.Acc); %#ok<NASGU>
        ...
        for bt_i = 1:2
            ...
            for oc_i = 1:2
                ...
                if ~isempty(tf_power_fcz)
                    ...
                    grand_tf.FCz.(stage_names{s_i}).(bt).(oc_str).subj(end+1,1)  = subj_key;
    ...
    % ------------------------------------------------------------------
    if ~isempty(tf_power_fcz)
        plot_subject_tf(subj, subj_key, has_P, group_table, subj_rows, ...
...
if isempty(t_global)
    error('S10: no subjects were loaded. Check RUN_COHORTS, valid participant lists, epoch folders, and .set filenames.');
end
...
% --------------------------------------------------------------------------
function subject_info = build_subject_info(group_table, run_cohorts, kh_valid, rr_valid)
%BUILD_SUBJECT_INFO  Build a unique KH+RR subject list from the combined table.
%   Uses the table rather than hard-coding loops so that RR is included only
%   when Nc rows are present in group_feature_table_combined.mat.

sid = string(group_table.subj_id);
coh = string(group_table.cohort);
all_ids = unique(sid(~ismissing(sid) & sid ~= ""), 'stable');

subject_info = struct('subj_id', {}, 'cohort', {}, 'subj_num', {});
for i = 1:numel(all_ids)
    this_id = all_ids(i);
    rows = sid == this_id;
    this_cohort = upper(string(coh(find(rows, 1, 'first'))));

    if this_cohort == "" || ismissing(this_cohort)
        if startsWith(this_id, "Ox", 'IgnoreCase', true)
            this_cohort = "KH";
        elseif startsWith(this_id, "Nc", 'IgnoreCase', true)
            this_cohort = "RR";
        else
            continue
        end
    ...
    if ~any(strcmpi(run_cohorts, char(this_cohort)))
        continue
    end

    tok = regexp(char(this_id), '\d+', 'match', 'once');
    if isempty(tok)
        this_num = NaN;
    else
        this_num = str2double(tok);
    end

    if this_cohort == "KH" && ~ismember(this_num, kh_valid)
        continue
    elseif this_cohort == "RR" && ~ismember(this_num, rr_valid)
        continue
    end

    subject_info(end+1).subj_id  = char(this_id); %#ok<AGROW>
    subject_info(end).cohort     = char(this_cohort);
    subject_info(end).subj_num   = this_num;
end

fprintf('Wavelet subject list: %d subjects (%d KH, %d RR)\n', ...
    numel(subject_info), ...
    sum(strcmp({subject_info.cohort}, 'KH')), ...
    sum(strcmp({subject_info.cohort}, 'RR')));
end

% --------------------------------------------------------------------------
function EEG_loaded = load_first_existing_set(folder, candidates)
%LOAD_FIRST_EXISTING_SET  Load the first .set file that exists.
EEG_loaded = [];
...
% =========================================================================
function plot_subject_tf(subj, subj_key, has_P, group_table, subj_rows, ...
